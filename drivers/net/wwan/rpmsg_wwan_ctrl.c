...
static int rpmsg_wwan_ctrl_probe(struct rpmsg_device *rpdev)
{
	 ...
	 * Make WWAN ports belong to parent device, so they show up as separate
	 * ports for the same WWAN device.
	 */
	port = wwan_create_port(rpdev->dev.parent, rpdev->id.driver_data,
