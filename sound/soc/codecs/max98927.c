...
static int max98927_info_rivision_ctl(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_info *uinfo);
static int max98927_get_rivision_ctl(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol);
static int max98927_set_rivision_ctl(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol);
                     ...
                     loff_t *ppos)
{
	...
	if (ret == 0) {
        ...
        if (!rdc_check_valid(impedance_l)) {
            impedance_l = 0xCACACACA;   //calibration failed specail code
            *payload = 0xCACACACA;
        }
		...
		if (max989xx->mono_stereo == 3) {
			if (!rdc_check_valid(impedance_r)) {
				impedance_r = 0xCACACACA;   //calibration failed specail code
...
static struct snd_kcontrol_new max98927_at_controls[] = {
	{
		...
		.info = max98927_info_rivision_ctl,
		.get =  max98927_get_rivision_ctl,
		.put =  max98927_set_rivision_ctl,
	},
};

static ssize_t max98927_state_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t max98927_state_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	struct max98927_priv *max98927 = snd_soc_component_get_drvdata(codec);
...
{
	u32  ret = 0;
 	//struct snd_soc_component *codec = w->codec;
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_err("%s\n", __func__);
	ret = regmap_read(max98927->regmap[0], MAX98927_REV_ID , &reg);
	if (ret < 0) {
...
static int max98927_probe(struct snd_soc_component *codec)
{
	struct max98927_priv *max98927 = snd_soc_component_get_drvdata(codec);
    ...
    snd_soc_dapm_ignore_suspend(dapm, "HiFi Capture");
		...
		snd_soc_add_component_controls(max98927->codec, max98927_at_controls,
				   ARRAY_SIZE(max98927_at_controls));
		...
		const struct i2c_device_id *id)
{
	...
	if(presence){
		...
		if(max98927->dev == NULL){
            ...
            pr_err("****** %s snd_soc_register_component id =%d\n",__func__,(int)(id->driver_data));
			if (ret < 0) {
		...
		ret = sysfs_create_file(&i2c->dev.kobj, &max98927_state_attr.attr);
    if(ret < 0)
    {
